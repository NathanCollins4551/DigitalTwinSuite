        ...
        for di, d in enumerate(detections):
            ...
            if tid == -1:
                ...
                self.tracks[tid] = {
                    ...
                    "center": (cx, cy),
                    "bbox": d["bbox"],
                    "smooth_bbox": list(d["bbox"]),   # smoothed bbox for stable display
                    "conf": d["conf"],
                    "zone_id": new_zone,
                    "prev_zone_id": None,
                ...
                # Smooth bbox with EMA (alpha=0.3: 30% new, 70% old) to reduce jitter
                alpha = 0.3
                sb = t.get("smooth_bbox") or list(d["bbox"])
                t["smooth_bbox"] = [
                    int(alpha * n + (1 - alpha) * s)
                    for n, s in zip(d["bbox"], sb)
                ]
            ...
            # Add to output list using smoothed bbox
            t2 = self.tracks[tid]
            tracks_out.append({
                "track_id": tid,
                "label": t2["label"],
                "conf": t2["conf"],
                "bbox": tuple(t2.get("smooth_bbox") or t2["bbox"]),
                "zone_id": t2.get("zone_id"),
                "prev_zone_id": t2.get("prev_zone_id"),
                "detected": True,   # actively matched this frame
            })

        # Include alive unmatched tracks in output so bbox persists between missed detections
        for tid, t in self.tracks.items():
            if t.get("matched"):
                continue
            ...
            if age <= self.max_age_frames:
                tracks_out.append({
                    ...
                    "label": t["label"],
                    "conf": t["conf"],
                    "bbox": tuple(t.get("smooth_bbox") or t["bbox"]),
                    "zone_id": t.get("zone_id"),
                    "prev_zone_id": t.get("prev_zone_id"),
                    "detected": False,  # ghost — kept alive but not seen this frame
                })
